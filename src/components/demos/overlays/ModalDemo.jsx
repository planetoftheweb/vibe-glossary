...
export default function ModalDemo({ activeOptions }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full relative w-full p-8">
      ...
      {isOpen && (
        <div className={`absolute inset-0 z-50 flex items-center justify-center p-8 ${isBlur ? 'bg-black/60 backdrop-blur-sm' : 'bg-black/50'} ${isAnim ? 'animate-fade-in' : ''}`}>
          <div className={`bg-white dark:bg-zinc-800 rounded-2xl shadow-2xl w-full border border-zinc-200 dark:border-zinc-700 flex flex-col ${isLarge ? 'max-w-3xl h-[28rem]' : 'max-w-md'} ${isAnim ? 'animate-zoom-in' : ''}`}>
