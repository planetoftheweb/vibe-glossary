...
export default function RichTextDemo({ activeOptions }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full w-full bg-zinc-50 dark:bg-zinc-900/50 p-4">
      <div className="w-full max-w-lg bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl shadow-sm overflow-hidden">
        {hasToolbar && (
          <div className="flex flex-wrap items-center gap-0.5 p-2 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-800/50">
            {TOOLBAR_GROUPS.map((group, gi) => (
              <div key={gi} className="flex items-center">
                {group.map(btn => {
                  const Icon = btn.icon;
                  return (
                    <button key={btn.label} className={`p-1.5 rounded-md transition-colors ${btn.active ? 'bg-zinc-200 dark:bg-zinc-700 text-zinc-900 dark:text-white' : 'text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-700 hover:text-zinc-700 dark:hover:text-zinc-300'}`} title={btn.label}>
                      <Icon size={15} />
                    </button>
                  );
                })}
                {gi < TOOLBAR_GROUPS.length - 1 && <div className="w-px h-5 bg-zinc-200 dark:bg-zinc-700 mx-1" />}
              </div>
            ))}
          </div>
        )}
        <div className="p-4 min-h-[200px]">
          {isMarkdown ? (
            <pre className="text-sm text-zinc-700 dark:text-zinc-300 font-mono leading-relaxed whitespace-pre-wrap">
          ...
          <div className="w-0.5 h-5 bg-zinc-900 dark:bg-white animate-pulse inline-block ml-0.5" />
        </div>
