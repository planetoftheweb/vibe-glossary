...
const makeExplore = (overrides = {}) => ({
  ...
  progress: { visited: 2, copied: 1, total: 44, percent: 5 },
...
describe('progress display', () => {
  it('renders "{visited}/{total} explored" text', () => {
    ...
    render(<ExploreBar {...props} />);
    expect(screen.getByText('2/44 explored')).toBeInTheDocument();
  });

  it('reflects custom visited/total values', () => {
    const props = defaultProps({
      progress: { visited: 10, copied: 3, total: 44, percent: 23 },
    });
    render(<ExploreBar {...props} />);
    expect(screen.getByText('10/44 explored')).toBeInTheDocument();
  });
});
...
describe('prompts copied count', () => {
  ...
  it('reflects a different copied count', () => {
    const props = defaultProps({
      progress: { visited: 2, copied: 5, total: 44, percent: 5 },
    });
    render(<ExploreBar {...props} />);
